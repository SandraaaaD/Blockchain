...
pub fn gen_idl_type(
    ...
    match ty {
        ...
        syn::Type::Path(path) => {
            ...
            // source_file() is only available when proc-macro2 wraps the real
            // proc_macro crate (wrap_proc_macro cfg), i.e. inside an actual
            // proc-macro compilation unit.  The IDL build is a regular binary
            // so wrap_proc_macro is NOT set — guard both conditions.
            #[cfg(all(procmacro2_semver_exempt, wrap_proc_macro))]
            {
