...
try {
  ...
  console.error('[DB] Не може да се поврзе со PostgreSQL:', err.message);
  console.error(`
Провери backend-node/.env — додади (или поправи) ги овие редови:

  PGHOST=127.0.0.1
  PGPORT=5432
  PGUSER=postgres
  PGPASSWORD=твојата_postgres_лозинка
  PGDATABASE=escrow_db

(Ако \"psql -U postgres\" прашува лозинка, мора да биде ИСТА со PGPASSWORD.)

Пробај и да ја избришеш линијата DATABASE_URL — тогаш се користат PG* параметрите.
`);
  process.exit(1);
}
