  ...
  return (
    <Layout>
      <div className="space-y-6">
        {/* Header */}
        <div>
          <button onClick={() => navigate(-1)} className="flex items-center gap-1.5 text-gray-500 hover:text-gray-700 mb-4 text-sm transition-colors">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="flex items-center gap-4 mt-2 text-sm text-gray-400">
                <span>Budget: <strong className="text-gray-700">${project.budget.toLocaleString()} USDC</strong></span>
                {project.deadlineDays && <span>Deadline: <strong className="text-gray-700">{project.deadlineDays} days</strong></span>}
                {isDeveloper && <span>Client: <strong className="text-gray-700">{project.client?.fullName || project.clientEmail}</strong></span>}
                {isClient && <span>Developer: <strong className="text-gray-700">{project.developer.fullName}</strong></span>}
              </div>
            </div>
        ...
        {isDeveloper && project.status === 'ACTIVE' && project.milestones.length > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-blue-900">The client accepted the invitation — you can start working.</p>
        ...
        {isDeveloper && project.status === 'FUNDED' && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-green-800">Escrow funded!</p>
              <p className="text-sm text-green-600">The client has deposited the funds. You can start working on the milestones.</p>
            </div>
          </div>
        )}

        {isClient && awaitingClient.length > 0 && (
          <div className="rounded-xl border-2 border-amber-400 bg-amber-50 p-4 flex flex-col sm:flex-row sm:items-center gap-4">
            <ClipboardCheck className="text-amber-600 shrink-0" size={28} />
        ...
        {isDeveloper && awaitingClient.length > 0 && (
          <div className="rounded-xl border border-sky-200 bg-sky-50 p-4 flex gap-3">
            <Send className="text-sky-600 shrink-0" size={22} />
            <div>
              <p className="font-medium text-sky-900">Submitted — waiting on the client</p>
            ...
            <div className="card">
              ...
              <dl className="space-y-3">
                ...
                <div className="flex justify-between">
                  ...
                  <dd className="text-sm font-medium">${project.budget.toLocaleString()} USDC</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-sm text-gray-500">Deadline</dt>
                  <dd className="text-sm font-medium">{project.deadlineDays || '—'} days</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-sm text-gray-500">Milestones</dt>
                  <dd className="text-sm font-medium">{completedMilestones} / {project.milestones.length} completed</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-sm text-gray-500">Requirements</dt>
                  <dd className="text-sm font-medium">{project.requirements.length} items</dd>
                </div>
              </dl>
            </div>
                  ...
                  {sortedMilestones.map((m) => (
                    <div key={m.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400">${m.amount.toLocaleString()}</p>
                      </div>
        ...
        {tab === 'requirements' && (
          <div className="card max-w-2xl">
            ...
            {requirements.length === 0 ? (
              <p className="text-gray-400 text-sm">No requirements defined yet</p>
            ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-medium text-blue-700 mt-1">${milestone.amount.toLocaleString()} USDC</p>
                      ...
                      {milestone.submission && (
                        <div className="mt-3 p-3 bg-blue-50 rounded-lg">
                          <p className="text-xs font-medium text-blue-700 mb-1">Submission:</p>
                          ...
                          {milestone.submission.notes && (
                            <p className="text-xs text-gray-600 mt-1">{milestone.submission.notes}</p>
                          )}
                        </div>
                      )}
                      ...
                      {milestone.aiFeedback && (
                        <div className="mt-3 p-3 bg-red-50 rounded-lg">
                          <p className="text-xs font-medium text-red-700 mb-1">AI Feedback:</p>
                          <p className="text-xs text-red-600">{milestone.aiFeedback}</p>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              <>
                {!showMilestoneForm ? (
                  ...
                  <div className="card border-2 border-dashed border-blue-200">
                    <h4 className="font-semibold text-gray-900 mb-4">New Milestone</h4>
